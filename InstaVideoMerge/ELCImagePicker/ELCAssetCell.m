...
- (void)setAssets:(NSArray *)assets
{
    ...
    for (int i = 0; i < [_rowAssets count]; ++i) {
        ...
        if (i < [_overlayViewArray count]) {
            ...
            if (overlayImage == nil) {
                overlayImage = [UIImage imageNamed:@"Overlay12.png"];
            }
            ELCOverlayImageView *overlayView = [[ELCOverlayImageView alloc] initWithImage:overlayImage];
            [_overlayViewArray addObject:overlayView];
            overlayView.layer.borderWidth = 2;
            overlayView.layer.borderColor = (__bridge CGColorRef _Nullable)([UIColor redColor]);
...
//    }
//    
//   // CGRect frame = CGRectMake(startX, 2, totalWidth/4-5, totalWidth/4-5);
//    
